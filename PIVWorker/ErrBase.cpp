...
CErrBase::CErrBase()
{
	...
	// Для № слова
	AddError(NumWord, "[ \t\n]*$", _T("Значение в поле отсутствует."));
	...
	AddError(NumWord, "^[ \t\n]*,[ \t]?[0-9]+$", _T("Отсутствует значение до запятой."));
	AddError(NumWord, "^[0-9]+,[ \t]?[ \t\n]*$", _T("Отсутствует значение после запятой."));
	AddError(NumWord, "^[ \t\n]*,[ \t]?[ \t\n]*$", _T("Отсутствуют значения до и после запятой."));
	...
	AddError(Bits, "", _T("Значение в поле содержит более двух промежутков."));
